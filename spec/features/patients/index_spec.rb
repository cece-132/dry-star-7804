...
RSpec.describe Patient do
  describe 'Patient Index page' do
    ...
    it 'Lists all the patients older than 18, in alpha order' do
      ...
      @pt_3 = Patient.create!(name: "Patient 3", age: 19)
      @pt_4 = Patient.create!(name: "Patient 4", age: 39)
      @pt_5 = Patient.create!(name: "Patient 5", age: 45)
      @pt_6 = Patient.create!(name: "Patient 6", age: 19)
      ...
      expect(@pt_1.name).to appear_before(@pt_2.name)
      expect(@pt_2.name).to appear_before(@pt_3.name)
      expect(@pt_3.name).to appear_before(@pt_4.name)
      expect(@pt_4.name).to appear_before(@pt_5.name)
      expect(@pt_5.name).to appear_before(@pt_6.name)
      expect(@pt_6.name).to appear_before(@pt_7.name)
      expect(@pt_7.name).to appear_before(@pt_8.name)
      expect(@pt_8.name).to_not appear_before(@pt_1.name)
    end

    it 'only list patients older than 18' do
      @hospital_1 = Hospital.create!(name: "Hospital 1")
      @hospital_2 = Hospital.create!(name: "Hospital 2")
      ...
      expect(page).to_not have_content(@pt_3)
      expect(page).to_not have_content(@pt_5)
      expect(page).to_not have_content(@pt_7)
    end
  end
end